...
sub authenticateUser {
	...
	my $res = makeRequest(1, [$uname, $upswd]);
	my @responseData;
	if ($res) {
...
sub downloadPythonBinary {
	...
	my $downlURL = $AppConfig::evsDownloadsPage;
	if ($AppConfig::appType eq "IDrive") {
	...
	$pybin =~ s/\.zip//g;
	$pybin = getECatfile($pybin, "bin");
	chmod 0777, getECatfile($appPath, $AppConfig::idrivePythonPath);
	my $cppytbin = Common::updateLocaleCmd(("cp -rf $pybin " . getECatfile($appPath, $AppConfig::idrivePythonPath)));
	`$cppytbin`;
...
# Objective				: Check if the installed python binary is compatible with this device.
...
sub hasPythonBinary {
	...
	if (-f $pybin) {
		...
		my $idrivepyver = updateLocaleCmd("$pybin -v");
...
#****************************************************************************************************/
sub makeRequest {
	unless (-f getCatfile(getAppPath(), "Idrivelib", "python", "bin", "idrivescripts")) {
		return '';
	}
	...
	if (getUserConfiguration('PROXYIP')) {
		my $proxy = getUserConfiguration('PROXYIP');
	...
	my $cmd = getECatfile(getAppPath(), "Idrivelib", "python", "bin", "idrivescripts");
	$cmd .= " $_[0] $filename";
	my $res = `$cmd`;
	if (-f $file) {
		unlink($file);
...
sub setBackupToLocation {
	if (getUserConfiguration('DEDUP') eq 'on') {
		...
		if ($responseData[0]->{'STATUS'} eq AppConfig::SUCCESS or ($responseData[0]->{'STATUS'} eq AppConfig::FAILURE and $responseData[0]->{'MSG'} =~ /file or folder exists/)){
			$backupLoc = removeMultipleSlashs($backupLoc);
			$backupLoc = removeLastSlash($backupLoc);
